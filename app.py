            ...
            if self.state == "searching":
                if '</think>' in self.buffer:
                    ...
                    if stripped_content and not stripped_content.startswith('<think') and not stripped_content.startswith('<resp'):
                        content_to_send = self.buffer; self.response_content += self.buffer; self.buffer = ""; self.state = "in_response"; break
                ...
                if '<response>' in self.buffer: self.buffer = self.buffer.replace('<response>', '', 1); self.state = "in_response"; continue
                content_to_send = self.buffer; self.response_content += self.buffer; self.buffer = ""; break
                ...
                self.response_content += content_to_send; self.buffer = ""
                if '</response>' in self.response_content: self.state = "finished"
                break
            elif self.state == "finished": self.buffer = ""; break
...
def download_logs():
    try: return send_file(LOG_FILE, as_attachment=True, download_name="proxy_logs.txt")
    except FileNotFoundError: return "No logs found yet.", 404
        ...
        if is_streaming:
            def generate_stream():
                ...
                try:
                        ...
                        if chunk:
                            ...
                            try:
                                ...
                                if 'error' in data:
                                    ...
                                    yield create_error_stream_chunk(f"Google AI Error: {error_message}"); yield 'data: [DONE]\n\n'; return
                                content_delta = ""; finish_reason = None
                                if 'candidates' in data and data['candidates']:
                                    candidate = data['candidates'][0]
                                    if 'finishReason' in candidate and candidate['finishReason'] not in ['STOP', 'MAX_TOKENS']:
                                        block_reason_detected = True; log_data += "--- BLOCKED BY GOOGLE ---\nReason: " + candidate['finishReason'] + "\n"
                                        yield create_error_stream_chunk(f"Google AI blocked the response. Reason: {candidate['finishReason']}"); yield 'data: [DONE]\n\n'; return
                                ...
                                if content_to_send:
                                    has_sent_data = True; final_sent_text += content_to_send; last_chunk_time = time.time()
                    ...
                    yield create_error_stream_chunk(f"Error during streaming: {e}"); yield 'data: [DONE]\n\n'
                finally:
            ...
            if response.status_code != 200:
                ...
                if google_response and 'error' in google_response:
                    error_detail = google_response['error'].get('message', response.text[:200]); error_msg = f"{error_msg} - {error_detail}"
                log_data += "--- GOOGLE ERROR ---\n" + error_msg + "\n"; write_log(log_data)
            ...
            candidate = google_response['candidates'][0]; content = ""
            if 'content' in candidate and 'parts' in candidate['content']:
                for part in candidate['content']['parts']:
            ...
            log_data += "--- FINAL TEXT SENT TO JANITOR ---\n" + content + "\n"; write_log(log_data)
            janitor_response = {"id": f"chatcmpl-{int(time.time())}", "object": "chat.completion", "created": int(time.time()), "model": selected_model, "choices": [{"index": 0, "message": {"role": "assistant", "content": content}, "finish_reason": candidate.get('finishReason', 'stop')}], "usage": google_response.get('usageMetadata', {})}
